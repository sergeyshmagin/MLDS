...
Этот модуль импортируется и в обучающей тетради, и при загрузке модели joblib-ом.
Поэтому он должен существовать в путях импорта на проде (рядом с пайплайном
или в PYTHONPATH).
"""
...
class ManualStandardScaler(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        # ddof=0 повторяет поведение sklearn.StandardScaler;
        # нулевую дисперсию заменяем на 1, чтобы не получить NaN.
...
def build_features(visits, ads, surf, cloud, primary=None):
    ...
    Возвращает:
      числовые: n_events, sessions_per_day, 3 доли времени суток, 19 долей категорий;
      категориальные: ads_activity, surf_depth, cloud_usage, primary_device, peak_daytime.

    Если primary не передан — primary_device заполняется маркером "unknown".
    """
    v = visits.copy()
    ...
    # Среднее число сессий в день. clip защищает от деления на ноль
    # на случай пользователя без активных дней.
    base["sessions_per_day"] = base["n_sessions"] / base["n_active_days"].clip(lower=1)
    # n_sessions и n_active_days свернули в sessions_per_day по правилу «A, B, A/B»
